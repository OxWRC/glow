...
def get_admin_credentials(
    ...
    """Read the ODK Central dashboard login generated by activate-stack.sh.

    Sudo is required: activate-stack.sh writes the file chmod 600, owned by
    the runner's service user.
    """
    output = run_ssm_command_capturing_output(
        instance_id,
        region,
        ["sudo cat /var/lib/glow/.deploy/.env.admin"],
    ...
    email = values.get("ODK_ADMIN_EMAIL")
    password = values.get("ODK_ADMIN_PASSWORD")
    if not email or not password:
        raise DeployError("admin credentials file is missing expected fields")
